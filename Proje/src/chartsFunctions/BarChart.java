...
public class BarChart implements PaintStrategy {
	//bar grafiğinin çizimi
	...
	protected double findMax(ChartStats stats) {
		double max = 0;
		for(Stat s: stats) {
			if (max < s.getdeger().doubleValue()) max = s.getdeger().doubleValue();
		}
		return max;
	}
	...
	public void paint(Graphics g, ChartStats stats, int height, int width) {
		...
		for(Stat stat: stats) {
			...
			drawSign(g, stat.getbaslik(), columnWidth, startPos, height);
