...
public class PieChart implements PaintStrategy {
	//pie chart grafiğinin çizimi
	...
	protected int getLegendWidth(ChartStats stats, int size, FontMetrics fm) {
		int legendWidth = 0;
		for(Stat stat: stats) {
			legendWidth += 4*size + INTERVAL + fm.stringWidth(stat.getbaslik());
		}
	...
	protected void drawMainContent(Graphics g, ChartStats stats, Point start, Point legend, FontMetrics fm, double sum, int legendSize, int realHeight, int realWidth) {
		...
		for(Stat stat: stats) {
			...
			float degrees = (float) (360 * stat.getdeger().doubleValue() / sum);
			...
			g.drawString(stat.getbaslik(), legend.x + 2*legendSize, legend.y + legendSize / 2);
			legend.x += 4*legendSize + INTERVAL + fm.stringWidth(stat.getbaslik());
	...
	protected void drawRest(Graphics g, int startAngle, ChartStats stats, Point start, int realHeight, int realWidth) {
		if (startAngle > -270) {
			Color last = stats.getStat(stats.getSize()-1).getrenk();
			g.setColor(last);
