...
public class Pie3D extends PieChart {
	//3 boyutlu pie grafiğinin çizimi
	...
	private void drawLeftBorder(Graphics g, ChartStats stats, Point center, double sum, int rx, int ry, int pieHeight) {
		...
		for(int i = stats.getSize() - 1; i >= 0; i--) {
			...
			float degrees = (float) (360 * stat.getdeger().doubleValue() / sum);
			rad -= 2 * Math.PI * stat.getdeger().doubleValue() / sum;
			angle -= Math.round(degrees);
			if (angle < 180) break;
			
			g.setColor(stats.getStat(i-1).getrenk().darker());
